...
import { 
  Clock, Smartphone, Truck, Shield, IndianRupee, Gift, Zap, Timer
} from 'lucide-react';
import FAQ from './FAQ';
...
export default function HeroSection({ onBookingClick }) {
  ...
  // ✅ Marquee items — same text as the Rapid Service card
  const marqueeItems = [
    { icon: <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />, text: 'Rapid Service' },
    { icon: <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />, text: 'Wash, dry & fold your laundry in just 3 hours' },
    { icon: <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />, text: 'Rapid Service' },
    { icon: <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />, text: 'Wash, dry & fold your laundry in just 3 hours' },
    { icon: <Zap className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />, text: 'Rapid Service' },
    { icon: <Clock className="w-4 h-4 sm:w-5 sm:h-5 text-white flex-shrink-0" />, text: 'Wash, dry & fold your laundry in just 3 hours' },
  ...
  return (
    <section className="w-full bg-gradient-to-b from-slate-50 to-white relative overflow-hidden mt-0">
      ...
      <div className="bg-white pt-8 sm:pt-12 lg:pt-16">
        ...
        {/* ✅ Marquee — full width, flush below special offer, same text as Rapid Service card */}
        <div className="w-full bg-[#1aa6b3] overflow-hidden py-3 sm:py-4">
          <div
            ...
            {[...Array(2)].map((_, dupIndex) => (
              <div key={dupIndex} className="flex items-center">
                {marqueeItems.map((item, index) => (
                  <span
                    key={`${dupIndex}-${index}`}
                    className="flex items-center gap-2 text-white font-semibold text-sm sm:text-base md:text-lg px-8 sm:px-12 flex-shrink-0"
                  >
                    {item.icon}
                    {item.text}
                    <span className="ml-8 sm:ml-12 text-white/30">|</span>
                  </span>
                ))}
              </div>
            ))}
      ...
      {/* Feature Cards — ✅ Rapid Service card RESTORED as first card */}
      <div className="bg-white py-8 sm:py-12 lg:py-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4 sm:gap-6 lg:gap-8">

            {/* ✅ CARD 1 - Rapid Service RESTORED with original text */}
            <article
            ...
            >
              ...
              <div className="relative z-10">
                ...
                {/* ✅ Original paragraph text restored */}
